...
const Castings = () => {
	...
	const formatDate = (date, longFormat = false) => {
		...
		if (longFormat) return `<span>${day}</span> ${monthNames[month - 1]} ${year}`;
				...
				{castings.map((casting, index) => (
					...
					>
						<div className={styles.casting_head}>
							<h3 dangerouslySetInnerHTML={{ __html: formatDate(casting.startDate, true) }}></h3>
							<h3>
								<span>{getRemainingDays(casting.startDate)}</span> zile ramase
							</h3>
						</div>
						...
						<div className={styles.casting_body}>
							...
							<div className={styles.casting_content}>
								...
								<div className={styles.casting_field}>
									<h3>Perioada:</h3>
									<h4>
										{formatDate(casting.startDate)} - {formatDate(casting.endDate)}
									</h4>
								</div>
								...
								<div className={styles.casting_field}>
									<h3>Durata:</h3>
									<h4>{getDuration(casting.startDate, casting.endDate)} zile</h4>
								</div>
